...
@section('content')
    <div class="main-content app-content">
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<h4 class="page-title">Edit Profit</h4>
						<ol class="breadcrumb">
										...
										<section>
											...
											<form action="{{ route('profits.update', $profit->id) }}" method="post" enctype="multipart/form-data">
												...
												<div class="control-group form-group">
													...
													<select name="event_id" class="form-select" aria-label="Default select example">
														@foreach ($event as $item)
															<option value="{{ $item->id }}" {{ $item->id == $profit->event_id ? 'selected' : '' }}>{{ $item->start }} - {{ $item->end }}</option>	
														@endforeach
													</select>
													@error('event_id')
												...
												<div class="control-group form-group">
													...
													<input type="number" class="form-control required @error('velue') is-invalid @enderror" value="{{ $profit->value }}" name="value">
													@error('value')
														<div class="alert alert-danger mt-2">
															{{ $message }}
														</div>
